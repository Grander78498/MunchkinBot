...
async def get_user(
    ...
    if user_id is not None:
        ...
        raise HTTPException(status_code=404, detail="Параметры сосали")

    user = result.scalar()
    if user is None:
        raise HTTPException(
            status_code=404,
            detail="Пользователя с таким tg_id или user_name не существует",
        )
...
async def get_active_user_game(user_id: int, session: AsyncSession) -> Game | None:
    """Информация об активной игре пользователя."""
    result = await session.execute(
        select(Game).where(Game.on_going).join(Munchkin).join(User).where(User.tg_id == user_id)
    )
    game = result.scalar()
    return game
...
async def get_action(action_id: int, session: AsyncSession) -> Action:
    ...
    if action is None:
        raise HTTPException(status_code=404, detail="Действия с таким id не существует")
    return action
...
async def get_condition(condition_id: int, session: AsyncSession) -> Condition:
    ...
    if condition is None:
        raise HTTPException(status_code=404, detail="Условия с таким id не существует")
    return condition
