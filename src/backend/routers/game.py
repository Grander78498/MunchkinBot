...
async def create_game(group: GroupBase, session: AsyncGameSession) -> Game:
    async with session.begin():
        result = await session.execute(select(Group).where(Group.tg_id == group.tg_id))
        group = result.scalar()
        game = Game(group=group)
        if game is None:
            raise HTTPException(status_code=404, detail="Группы с таким tg_id не существует")
        session.add(game)
...
async def create_munchkin(game_id: int, user: UserBase, session: AsyncGameSession) -> Munchkin:
    try:
        async with session.begin():
        ...
        raise HTTPException(status_code=404, detail="Пользователь уже есть в данной игровой партии")
