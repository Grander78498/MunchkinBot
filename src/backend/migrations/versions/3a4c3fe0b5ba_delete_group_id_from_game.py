...
def upgrade() -> None:
    ...
    op.add_column('item', sa.Column('item_type_id', sa.Integer(), nullable=True))
    op.create_foreign_key(op.f('fk_item_item_type_id_itemtype'), 'item', 'itemtype', ['item_type_id'], ['id'])
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_constraint(op.f('fk_item_item_type_id_itemtype'), 'item', type_='foreignkey')
    ...
    op.add_column('game', sa.Column('group_id', sa.INTEGER(), autoincrement=False))
    op.create_foreign_key('fk_game_group_id_tg_group', 'game', 'tg_group', ['group_id'], ['tg_id'])
    # ### end Alembic commands ###
