...
from backend.database.link_models import ItemCondition, ActionCondition, CardsTransferCondition
...
class Condition(CustomSQLModel, table=True):
    ...
    actions: list["Action"] = lazy_relationship(back_populates="conditions", link_model=ActionCondition)
    items: list["Item"] = lazy_relationship(back_populates="conditions", link_model=ItemCondition)
    cards_transfers: list["CardsTransfer"] = lazy_relationship(back_populates="conditions", link_model=CardsTransferCondition)
