...
from backend.database.link_models import (ItemCondition, ActionCondition,
...
class Condition(CustomSQLModel, table=True):
    ...
    equal_type: EqualType = Field(
        sa_type=ENUM(EqualType))  #type: ignore [call-overload]

    value: "PossibleConditionValue" = lazy_relationship(
        back_populates="conditions")
    actions: list["Action"] = lazy_relationship(back_populates="conditions",
                                                link_model=ActionCondition)
    items: list["Item"] = lazy_relationship(back_populates="conditions",
...
class PossibleConditionValue(CustomSQLModel, table=True):
    ...
    field: "PossibleConditionField" = lazy_relationship(
