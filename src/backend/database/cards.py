...
from backend.database.link_models import (MunchkinCard, MunchkinItem,
                                          MonsterCombat, MunchkinStats,
                                          CardAction, ItemCondition,
                                          ActionMonster)
...
class CardBase(CustomSQLModel):
    ...
    card_type: CardType = Field(
        sa_type=ENUM(CardType))  # type: ignore[call-overload]
...
class Card(CardBase, table=True):
    ...
    actions: list["Action"] = lazy_relationship(back_populates="cards",
...
class GameCard(CustomSQLModel, table=True):
    ...
    source_type: SourceType = Field(
        sa_type=ENUM(SourceType))  # type: ignore[call-overload]
    open: bool
...
class MonsterBase(CustomSQLModel):
    ...
    monster_type_id: int | None = Field(default=None,
                                        foreign_key="monstertype.id")
...
class Monster(MonsterBase, table=True):
    ...
    combats: list["Combat"] = lazy_relationship(back_populates="monsters",
                                                link_model=MonsterCombat)
    monster_type: MonsterType | None = lazy_relationship(
        back_populates="monsters")
    actions: list["Action"] = lazy_relationship(back_populates="monsters",
...
class ItemBase(CustomSQLModel):
    ...
    item_property_id: int | None = Field(default=None,
                                         foreign_key="itemproperty.id")
...
class Item(ItemBase, table=True):
    ...
    conditions: list["Condition"] = lazy_relationship(back_populates="items",
                                                      link_model=ItemCondition)
